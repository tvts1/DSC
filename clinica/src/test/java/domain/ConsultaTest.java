...
public class ConsultaTest extends GenericTest {
    ...
    @Test
    public void testConsultasPorNomeMedico() {
        String jpql = "SELECT c FROM Consulta c WHERE c.medico.nome = :nome";
        
        List<Consulta> consultas = em.createQuery(jpql, Consulta.class)
                .setParameter("nome", "Dr. Carlos")
                .getResultList();

        assertFalse(consultas.isEmpty());
        assertEquals("Dr. Carlos", consultas.get(0).getMedico().getNome());
    }

    @Test
    public void testContarConsultasPacienteEspecifico() {
        String jpql = "SELECT COUNT(c) FROM Consulta c WHERE c.paciente.nome = :nome";
    ...
    @Test
    public void testConsultasPorNomeExame() {
        String jpql = "SELECT c FROM Consulta c JOIN c.exames e WHERE e.nome = :exameNome";
        
        List<Consulta> consultas = em.createQuery(jpql, Consulta.class)
                .setParameter("exameNome", "Hemograma")
                .getResultList();
    ...
    @Test
    public void testValorTotalPagoViaPix() {
        String jpql = "SELECT SUM(p.valorPago) FROM Pagamento p WHERE p.formaPagamento = :forma";
        
        Double totalPix = em.createQuery(jpql, Double.class)
                .setParameter("forma", "PIX")
                .getSingleResult();

        assertEquals(150.00, totalPix);
    }

    @Test
    public void testConsultasPorCidadeDoPaciente() {
        String jpql = "SELECT c FROM Consulta c WHERE c.paciente.endereco.cidade = :cidade";
        
        List<Consulta> consultasRecife = em.createQuery(jpql, Consulta.class)
                .setParameter("cidade", "Recife")
                .getResultList();

        assertFalse(consultasRecife.isEmpty());
    }
}
